    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (var item in data)
                Console.Write(item + " ");
            Console.WriteLine();
            var strings = new string[] { "One", "oTwo", "Data",
                "Dobroe", "2Ukraine", "Found", "Environment",
                "redivider", "азы", "буки", "веды", "глаголь",
                "добро", "истинно", "есть", "жизнь", "A", "B", "C" };
            ...
            Console.WriteLine($"1. First: {data.First(x => x > 0)}, End: {data.Last(x => x < 0)}");
            ...
            Console.WriteLine($"2: {data.FirstOrDefault(x => x > 0 && x % 10 == D)}");
            ...
            try
            {
                Console.WriteLine($"4: {strings.SingleOrDefault(x => x.Last() == C)}");
            }
            ...
            Console.WriteLine($"5: {strings.Where(x => x.Length > 1 && x.First() == C && x.Last() == C).Count()}");

            //6. Дана строковая последовательность strings. Найти сумму длин всех строк, входящих в данную последовательность.
            Console.WriteLine($"6: {strings.Sum(x => x.Length)}");
            ...
            Console.WriteLine($"7:Count: {data.Where(x => x < 0).Count()}");
            Console.WriteLine($"  Sum: {data.Where(x => x < 0).Sum()}");
            ...
            Console.WriteLine($"8:Count > 9 && < 100: {data.Where(x => x > 9 && x < 100).Count()}");
            Console.WriteLine($"  Srednee arifm.: {data.Where(x => x > 9 && x < 100).Sum() / data.Where(x => x > 9 && x < 100).Count()}");

            //9. Дана целочисленная последовательность data. 
            //Вывести ее минимальный положительный элемент или число 0, если последовательность не содержит положительных элементов.
            Console.WriteLine($"9:Min: {data.Where(x => x > 0).Min()}");

            //10. Дано целое число L (> 0) и строковая последовательность bigs.
            //Строки последовательности bigs содержат только заглавные буквы латинского алфавита. 
            //Среди всех строк из bigs, имеющих длину L, найти наибольшую (в смысле лексикографического порядка).
            //Вывести эту строку или пустую строку, если последовательность не содержит строк длины L.

            //11. Дана последовательность непустых строк strings. 
